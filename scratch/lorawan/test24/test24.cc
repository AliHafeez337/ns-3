/*
 * Scenario 3 - Simulation 1 - Attack: 
 1 ED, 10GW, 1NS, 1packet
...
main(int argc, char* argv[])
{
    ...
    allocator->Add(Vector(5000, 0, 0)); // ED
    ...
    for (int i = 0; i < 10; i++)
    {
      allocator->Add(Vector((rand() % 5000), (rand() % 5000), 100));
    }

    allocator->Add(Vector(5000, 5000, 0)); // NS
    ...
    //     Ptr<LoraNetDevice> gatewayDevice = gwNode->GetDevice(0)->GetObject<LoraNetDevice>();
        
    //     // Assign a custom MAC address to the gateway
    //     Mac48Address customMacAddress("01:23:45:67:89:ab");
    //     gatewayDevice->SetAddress(customMacAddress);

    //     NS_LOG_INFO("test24: mac address of GW: " << i << "  is: " << gatewayDevice->GetAddress());
    // }
